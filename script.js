...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initPreloader(); // Loading preloader
});
...
function initPreloader() {
    ...
    let progress = 0;
    let animationFrame;
    const duration = 2000; // 2 seconds for preloader animation
    ...
    function animatePreloader(currentTime) {
        ...
        // Update progress bar, logo fill, and text
        progressBar.style.width = `${progress}%`;
        progressText.textContent = `${progress}%`;

        // Update logo fill width to match progress
        if (logoFill) {
            logoFill.style.width = `${progress}%`;
        }

        if (progress < 100) {
            animationFrame = requestAnimationFrame(animatePreloader);
        } else {
            // Preloader complete - hide it
            setTimeout(() => {
    ...
    // Fallback: Ensure preloader hides even if animation fails or page loads faster
